	...
	$treeView.jstree({
		'core': {
			...
			'data': function (objNode, cb) {
				$.ajax({
				    url: app.conf.url.api.cdms.exam.category.getCategoryTree,
				    method: 'GET',
				    data: {
...
function initFuncBtnEvent(){
	$("#examCategoryAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		...
		params.operUrl = top.app.conf.url.apigateway + $("#examCategoryAdd").data('action-url');
		top.app.layer.editLayer('新增试题分类', ['710px', '350px'], '/cdms/exam/category-edit.html', params, function(){
   			//重新加载
			$treeView.jstree(true).refresh();
		});
    });
	$("#examCategoryEdit").click(function () {
		if(g_selectNode == null ){
		...
		params.parentNode = getSelNodeParent();
		params.allTreeData = g_allTreeData;
		params.operUrl = top.app.conf.url.apigateway + $("#examCategoryEdit").data('action-url');
		top.app.layer.editLayer('编辑试题分类', ['710px', '350px'], '/cdms/exam/category-edit.html', params, function(){
   			//重新加载列表
			$treeView.jstree(true).refresh();
		});
    });
	$("#examCategoryDel").click(function () {
		if(g_selectNode == null ){
			top.app.message.alert("请选择要删除的节点！");
			return;
		}
		var operUrl = top.app.conf.url.apigateway + $("#examCategoryDel").data('action-url');
