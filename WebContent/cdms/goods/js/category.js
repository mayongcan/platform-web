	...
	$treeView.jstree({
		'core': {
			...
			'data': function (objNode, cb) {
				$.ajax({
				    url: app.conf.url.api.cdms.goods.category.getCategoryTree,
				    method: 'GET',
				    data: {
...
function initFuncBtnEvent(){
	$("#goodsCategoryAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		...
		params.operUrl = top.app.conf.url.apigateway + $("#goodsCategoryAdd").data('action-url');
		top.app.layer.editLayer('新增商品分类', ['710px', '350px'], '/cdms/goods/category-edit.html', params, function(){
   			//重新加载
			$treeView.jstree(true).refresh();
		});
    });
	$("#goodsCategoryEdit").click(function () {
		if(g_selectNode == null ){
		...
		params.parentNode = getSelNodeParent();
		params.allTreeData = g_allTreeData;
		params.operUrl = top.app.conf.url.apigateway + $("#goodsCategoryEdit").data('action-url');
		top.app.layer.editLayer('编辑商品分类', ['710px', '350px'], '/cdms/goods/category-edit.html', params, function(){
   			//重新加载列表
			$treeView.jstree(true).refresh();
		});
    });
	$("#goodsCategoryDel").click(function () {
		if(g_selectNode == null ){
			top.app.message.alert("请选择要删除的节点！");
			return;
		}
		var operUrl = top.app.conf.url.apigateway + $("#goodsCategoryDel").data('action-url');
