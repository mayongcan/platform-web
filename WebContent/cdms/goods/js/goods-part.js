...
function initFunc(){
	...
	for (var i = 0; i < length; i++) {
		if(operRights[i].funcFlag.indexOf("goodsPart") != -1){
...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.api.cdms.goods.goods.getGoodsPartList,   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#goodsPartAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.goodsId = g_goodsId;
		params.operUrl = top.app.conf.url.apigateway + $("#goodsPartAdd").data('action-url');
		top.app.layer.editLayer('新增商品配件', ['710px', '240px'], '/cdms/goods/goods-part-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#goodsPartEdit").click(function () {
		...
		params.goodsId = g_goodsId;
		params.rows = rows[0];
		params.operUrl = top.app.conf.url.apigateway + $("#goodsPartEdit").data('action-url');
		top.app.layer.editLayer('编辑商品配件', ['710px', '240px'], '/cdms/goods/goods-part-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#goodsPartDel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		$.each(rows, function(i, rowData) {
			...
			idsList = idsList + rowData.partId;
    	});
		appTable.delData($table, $("#goodsPartDel").data('action-url'), idsList);
    });
	//返回数据类型页面
