...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.api.cdms.hsm.hospital.getList,   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	...
	$("#ruleTrainAdd").click(function () {
		...
		top.app.layer.editLayer('新增培训积分规则', ['710px', '330px'], '/cdms/rule/train-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#ruleTrainEdit").click(function () {
		...
		top.app.layer.editLayer('编辑培训积分规则', ['710px', '330px'], '/cdms/rule/train-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
