	...
	$treeView.jstree({
		'core': {
			...
			'data': function (objNode, cb) {
				$.ajax({
				    url: app.conf.url.api.cdms.merchant.industry.getIndustryTree,
				    method: 'GET',
				    data: {
...
function initFuncBtnEvent(){
	$("#industryAdd").click(function () {
		...
		params.operUrl = top.app.conf.url.apigateway + $("#industryAdd").data('action-url');
		top.app.layer.editLayer('新增行业信息', ['710px', '340px'], '/cdms/merchant/industry-edit.html', params, function(){
   			//重新加载
	...
	$("#industryEdit").click(function () {
		...
		params.parentNode = getSelNodeParent();
		params.allTreeData = g_allTreeData;
		params.operUrl = top.app.conf.url.apigateway + $("#industryEdit").data('action-url');
		top.app.layer.editLayer('编辑行业信息', ['710px', '340px'], '/cdms/merchant/industry-edit.html', params, function(){
   			//重新加载列表
			$treeView.jstree(true).refresh();
		});
    });
	$("#industryDel").click(function () {
		if(g_selectNode == null ){
			top.app.message.alert("请选择要删除的节点！");
			return;
		}
		var operUrl = top.app.conf.url.apigateway + $("#industryDel").data('action-url');
...
function showInfo(){
	if(g_selectNode != null){
		$('#tdIndustryName').text(g_selectNode.text);
		...
		if(g_selectNode.original != null && g_selectNode.original != undefined && 
			...
			$('#tdMemo').text(g_selectNode.original.attributes.memo == undefined ? "" : g_selectNode.original.attributes.memo);
			$('#tdDispOrder').text(g_selectNode.original.attributes.dispOrder == undefined ? "" : g_selectNode.original.attributes.dispOrder);
			$('#tbCreateDate').text(g_selectNode.original.attributes.createDate == undefined ? "" : g_selectNode.original.attributes.createDate);
		}
	}
}