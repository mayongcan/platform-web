...
var g_codeType = rales.writNecessity3_8, g_codeCurNum = "";
...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$("#totalCnt1").val(g_params.subRow.content.totalCnt1);
			$("#totalCnt2").val(g_params.subRow.content.totalCnt2);
		}
		//显示文书列表
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$("#tdTotalCnt1").text($.utils.getNotNullVal(g_params.subRow.content.totalCnt1));
			$("#tdTotalCnt2").text($.utils.getNotNullVal(g_params.subRow.content.totalCnt2));
		}
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/necessity/writ-pre3_8.html', params, function(){});
    });
	//提交
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		...
		data.totalCnt1 = $('#totalCnt1').val();
		data.totalCnt2 = $('#totalCnt2').val();
	}else{
