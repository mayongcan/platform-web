...
var g_codeType = rales.writOptional10_1, g_codeCurNum = "";
...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#caseCode').val(g_params.subRow.content.caseCode);
			...
			if(!$.utils.isEmpty(g_params.subRow.content.applyList)){
				var array = g_params.subRow.content.applyList.split(',');
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdCaseCode').text($.utils.getNotNullVal(g_params.subRow.content.caseCode));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre10_1.html', params, function(){});
    });
	//预览
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		...
		var applyList = "";
		for(var i = 1; i < 12;i++){
			if($('#apply' + i).prop('checked'))  applyList += "1,";
			else applyList += "0,";
		}
