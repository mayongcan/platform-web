...
var g_codeType = rales.writOptional11_3, g_codeCurNum = "";
...
function initView(){
	...
	top.app.addComboBoxOption($("#partiesSex1"), g_sexDict, true, ' ');
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#partiesSex1').val(g_params.subRow.content.partiesSex1);
			$('#partiesAge1').val(g_params.subRow.content.partiesAge1);
			$('#partiesAddr1').val(g_params.subRow.content.partiesAddr1);
			$('#partiesCertificateNo1').val(g_params.subRow.content.partiesCertificateNo1);
			
			$('#illegalContent').val(g_params.subRow.content.illegalContent);
			$('#protocol').val(g_params.subRow.content.protocol);
			$('#principal').val(g_params.subRow.content.principal);
			$('#handler').val(g_params.subRow.content.handler);
		}
		//显示文书列表
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdPartiesSex1').text($.utils.getNotNullVal(top.app.getDictName(g_params.subRow.content.partiesSex1, g_sexDict)));
			$('#tdPartiesAge1').text($.utils.getNotNullVal(g_params.subRow.content.partiesAge1));
			$('#tdPartiesAddr1').text($.utils.getNotNullVal(g_params.subRow.content.partiesAddr1));
			$('#tdPartiesCertificateNo1').text($.utils.getNotNullVal(g_params.subRow.content.partiesCertificateNo1));
			
			$('#illegalContent').text($.utils.getNotNullVal(g_params.subRow.content.illegalContent));
			$('#protocol').text($.utils.getNotNullVal(g_params.subRow.content.protocol));
			$('#principal').text($.utils.getNotNullVal(g_params.subRow.content.principal));
			$('#handler').text($.utils.getNotNullVal(g_params.subRow.content.handler));
		}
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre11_3.html', params, function(){});
    });
	//提交
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){	
		...
		data.partiesSex1 = $('#partiesSex1').val();
		data.partiesAge1 = $('#partiesAge1').val();
		data.partiesAddr1 = $('#partiesAddr1').val();
		data.partiesCertificateNo1 = $('#partiesCertificateNo1').val();
