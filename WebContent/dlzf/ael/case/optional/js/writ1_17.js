...
var g_codeType = rales.writOptional1_17, g_codeCurNum = "";
...
function initView(){
	...
	$('#divRegisterDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true, defaultDate: new Date()});
	$('#divCaseEndDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true, defaultDate: new Date()});
	$('#divArchiveDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true, defaultDate: new Date()});
	...
	if(g_params.type == 1){
		...
		$('#writCode').val(g_params.row.otherFlowParams.code);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#handleResult').val(g_params.subRow.content.handleResult);
			$('#handleOrg').val(g_params.subRow.content.handleOrg);
			$('#handleUser').val(g_params.subRow.content.handleUser);
			$('#registerDate').val(g_params.subRow.content.registerDate);
			$('#caseEndDate').val(g_params.subRow.content.caseEndDate);
			$('#archiveUser').val(g_params.subRow.content.archiveUser);
			$('#archiveDate').val(g_params.subRow.content.archiveDate);
			$('#saveDeadline').val(g_params.subRow.content.saveDeadline);
			$('#archiveCode').val(g_params.subRow.content.archiveCode);
			$('#archivePage').val(g_params.subRow.content.archivePage);
		}
		//显示文书列表
...
//		$('#content-top-print').css('right', '26%');
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdHandleResult').text($.utils.getNotNullVal(g_params.subRow.content.handleResult));
			$('#tdHandleOrg').text($.utils.getNotNullVal(g_params.subRow.content.handleOrg));
			$('#tdHandleUser').text($.utils.getNotNullVal(g_params.subRow.content.handleUser));
			$('#tdRegisterDate').text($.utils.getNotNullVal(g_params.subRow.content.registerDate));
			$('#tdCaseEndDate').text($.utils.getNotNullVal(g_params.subRow.content.caseEndDate));
			$('#tdArchiveUser').text($.utils.getNotNullVal(g_params.subRow.content.archiveUser));
			$('#tdArchiveDate').text($.utils.getNotNullVal(g_params.subRow.content.archiveDate));
			$('#tdSaveDeadline').text($.utils.getNotNullVal(g_params.subRow.content.saveDeadline));
			$('#tdArchiveCode').text($.utils.getNotNullVal(g_params.subRow.content.archiveCode));
			$('#tdArchivePage').text($.utils.getNotNullVal(g_params.subRow.content.archivePage));
		}
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		...
		data.handleResult = $('#handleResult').val();
		data.handleOrg = $('#handleOrg').val();
		data.handleUser = $('#handleUser').val();
		data.registerDate = $('#registerDate').val();
		data.caseEndDate = $('#caseEndDate').val();
		data.archiveUser = $('#archiveUser').val();
		data.archiveDate = $('#archiveDate').val();
		data.saveDeadline = $('#saveDeadline').val();
		data.archiveCode = $('#archiveCode').val();
		data.archivePage = $('#archivePage').val();
	}else{
