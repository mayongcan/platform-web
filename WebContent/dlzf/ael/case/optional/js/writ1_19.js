...
var g_codeType = rales.writOptional1_19, g_codeCurNum = "";
...
function initView(){
	$('#divSendDate').datetimepicker({locale: 'zh-CN', format: "YYYY-MM-DD HH:mm", allowInputToggle: true, defaultDate: new Date()});
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#checkUser').val(g_params.subRow.content.checkUser);
		}
		//显示文书列表
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdCheckUser').text($.utils.getNotNullVal(g_params.subRow.content.checkUser));
		}
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre1_19.html', params, function(){});
    });
	//提交
