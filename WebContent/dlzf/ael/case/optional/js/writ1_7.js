...
var g_codeType = rales.writOptional1_7, g_codeCurNum = "";
...
function initView(){
	$('#divCreateDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD HH:mm', allowInputToggle: true, defaultDate: new Date()});
	//1新增 2编辑 3查看
	if(g_params.type == 1){
		//增加表单验证
		formValidate();
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#sampleAddr').val(g_params.subRow.content.sampleAddr);
			...
			$('#sampleObj').val(g_params.subRow.content.sampleObj);
			$('#sampleNum').val(g_params.subRow.content.sampleNum);
			$('#sampleType').val(g_params.subRow.content.sampleType);
			$('#sampleDetail').val(g_params.subRow.content.sampleDetail);
			$('#sampleResult').val(g_params.subRow.content.sampleResult);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdSampleAddr').text($.utils.getNotNullVal(g_params.subRow.content.sampleAddr));
			...
			$('#tdSampleObj').text($.utils.getNotNullVal(g_params.subRow.content.sampleObj));
			$('#tdSampleNum').text($.utils.getNotNullVal(g_params.subRow.content.sampleNum));
			$('#tdSampleType').text($.utils.getNotNullVal(g_params.subRow.content.sampleType));
			$('#tdSampleDetail').text($.utils.getNotNullVal(g_params.subRow.content.sampleDetail));
			$('#tdSampleResult').text($.utils.getNotNullVal(g_params.subRow.content.sampleResult));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre1_7.html', params, function(){});
    });
	//提交
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){	
		...
		data.sampleAddr = $('#sampleAddr').val();
		...
		data.sampleObj = $('#sampleObj').val();
		data.sampleNum = $('#sampleNum').val();
		data.sampleType = $('#sampleType').val();
		data.sampleDetail = $('#sampleDetail').val();
		data.sampleResult = $('#sampleResult').val();
		data.memo = $('#memo').val();
	}else{
