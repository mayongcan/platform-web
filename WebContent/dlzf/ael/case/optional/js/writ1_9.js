...
var g_codeType = rales.writOptional1_9, g_codeCurNum = "";
...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#applyDate').val(g_params.subRow.content.applyDate);
			$('#applyAddr').val(g_params.subRow.content.applyAddr);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdApplyDate').text($.utils.getNotNullVal(g_params.subRow.content.applyDate));
			$('#tdApplyAddr').text($.utils.getNotNullVal(g_params.subRow.content.applyAddr));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre1_9.html', params, function(){});
    });
	//提交
