...
var g_codeType = rales.writOptional7_1, g_codeCurNum = "";
...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#partiesContacts').val(g_params.subRow.content.partiesContacts);
			$('#partiesAddress').val(g_params.subRow.content.partiesAddress);
			$('#partiesZip').val(g_params.subRow.content.partiesZip);
			$('#partiesPhone').val(g_params.subRow.content.partiesPhone);
			$('#hearingDetail').val(g_params.subRow.content.hearingDetail);
			$('#memo').val(g_params.subRow.content.memo);
			$('#suggest').val(g_params.subRow.content.suggest);
		}
		//显示文书列表
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdPartiesContacts').text($.utils.getNotNullVal(g_params.subRow.content.partiesContacts));
			$('#tdPartiesAddress').text($.utils.getNotNullVal(g_params.subRow.content.partiesAddress));
			...
			$('#tdHearingDetail').text($.utils.getNotNullVal(g_params.subRow.content.hearingDetail));
			$('#tdMemo').text($.utils.getNotNullVal(g_params.subRow.content.memo));
			$('#tdSuggest').text($.utils.getNotNullVal(g_params.subRow.content.suggest));
		}
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre7_1.html', params, function(){});
    });
	//预览
