...
var g_codeType = rales.writOptional7_3, g_codeCurNum = "";
...
function initView(){
	...
	$('#divHearingDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD HH:mm', allowInputToggle: true, defaultDate: new Date()});
	$('#divPostponeDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true, defaultDate: new Date()});
	//1新增 2编辑 3查看
	if(g_params.type == 1){
		//增加表单验证
		formValidate();
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#hearingDate').val(g_params.subRow.content.hearingDate);
			$('#hearingAddr').val(g_params.subRow.content.hearingAddr);
			$('#hearingUser1').val(g_params.subRow.content.hearingUser1);
			$('#hearingUser2').val(g_params.subRow.content.hearingUser2);
			$('#hearingUser3').val(g_params.subRow.content.hearingUser3);
			$('#postponeDate').val(g_params.subRow.content.postponeDate);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdHearingDate').text($.utils.getNotNullVal(g_params.subRow.content.hearingDate));
			$('#tdHearingAddr').text($.utils.getNotNullVal(g_params.subRow.content.hearingAddr));
			$('#tdHearingUser1').text($.utils.getNotNullVal(g_params.subRow.content.hearingUser1));
			$('#tdHearingUser2').text($.utils.getNotNullVal(g_params.subRow.content.hearingUser2));
			$('#tdHearingUser3').text($.utils.getNotNullVal(g_params.subRow.content.hearingUser3));
			$('#tdPostponeDate').text($.utils.getNotNullVal(g_params.subRow.content.postponeDate));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre7_3.html', params, function(){});
    });
	//提交
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){	
		...
		data.hearingAddr = $('#hearingAddr').val();
		data.hearingUser1 = $('#hearingUser1').val();
		data.hearingUser2 = $('#hearingUser2').val();
		data.hearingUser3 = $('#hearingUser3').val();
		data.postponeDate = $('#postponeDate').val();
