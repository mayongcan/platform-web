...
var g_codeType = rales.writOptional9_1, g_codeCurNum = "";
...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#totalCnt').val(g_params.subRow.content.totalCnt);
			$("input[type='radio'][name=handleType][value=" + g_params.subRow.content.handleType + "]").attr("checked",true);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdTotalCnt').text($.utils.getNotNullVal(g_params.subRow.content.totalCnt));
			$("input[type='radio'][name=handleType][value=" + g_params.subRow.content.handleType + "]").attr("checked",true);
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre9_1.html', params, function(){});
    });
	//预览
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		...
		data.totalCnt = $('#totalCnt').val();
		data.handleType = $('#tdHandleType input:radio:checked').val();
