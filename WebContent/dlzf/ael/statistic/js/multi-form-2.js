...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/rales/ael/statistic/getStatisticCaseProcedureList",   		//请求后台的URL（*）
...
//	        }
	...
	for(var i = 0; i < data.length; i++){
		nameData[i] = top.app.getDictName(data[i].name, g_caseProcedureDict);
	}
	//数组去重
			...
			for(var j = 0; j < xAxisData.length; j++){
				...
				for(var k = 0; k < data.length; k++){
					if(!$.utils.isNull(data[k].name)) {
						var tmpName = top.app.getDictName(data[k].name, g_caseProcedureDict);
...
function tableFormatName(value,row,index){
	return appTable.tableFormatDictValue(g_caseProcedureDict, value);
}