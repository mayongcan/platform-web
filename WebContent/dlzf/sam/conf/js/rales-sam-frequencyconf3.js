...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#ralesSamFrequencyconfAdd3").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		...
		params.operUrl = top.app.conf.url.apigateway + $("#ralesSamFrequencyconfAdd3").data('action-url');
		top.app.layer.editLayer('新增', ['710px', '460px'], '/rales/sam/conf/rales-sam-frequencyconf-edit3.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function ralesSamFrequencyconfEdit3(id, url){
	...
	top.app.layer.editLayer('编辑', ['710px', '460px'], '/rales/sam/conf/rales-sam-frequencyconf-edit3.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function ralesSamFrequencyconfDel3(id, url){
