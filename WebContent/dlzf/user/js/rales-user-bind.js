...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/rales/user/getList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#ralesUserBindAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.loginUrlDict = g_loginUrlDict;
		params.operUrl = top.app.conf.url.apigateway + $("#ralesUserBindAdd").data('action-url');
		top.app.layer.editLayer('绑定账号', ['710px', '280px'], '/rales/user/rales-user-bind-edit.html', params, function(){
   			//重新加载列表
...
function ralesUserBindDel(id, url){
	appTable.delData($table, url, id + "");
}

function formatLoginUrl(value,row,index){
	return appTable.tableFormatDictValue(g_loginUrlDict, value);
}
