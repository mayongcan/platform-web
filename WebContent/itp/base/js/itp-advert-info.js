 ...
 */
function initView(){
	g_positionDict = top.app.getDictDataByDictTypeValue('ITP_ADVERT_POSITIOIN');
...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/base/getAdvertInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpAdvertInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.positionDict = g_positionDict;
		params.operUrl = top.app.conf.url.apigateway + $("#itpAdvertInfoAdd").data('action-url');
		top.app.layer.editLayer('新增APP广告位', ['710px', '400px'], '/itp/base/itp-advert-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	// 排序
	$("#itpAdvertInfoSaveOrder").click(function () {
		//获取所有项,并根据当前排序设置排序ID
		...
		appTable.postData($table, $("#itpAdvertInfoSaveOrder").data('action-url'), JSON.stringify(list),
				"确定要保存当前的排序结果？", "数据保存成功！");
    });
}
...
function itpAdvertInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑APP广告位', ['710px', '400px'], '/itp/base/itp-advert-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function itpAdvertInfoDel(id, url){
