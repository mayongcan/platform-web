...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/base/getColorInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpColorInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#itpColorInfoAdd").data('action-url');
		top.app.layer.editLayer('新增商品颜色', ['710px', '220px'], '/itp/base/itp-color-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function itpColorInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑商品颜色', ['710px', '220px'], '/itp/base/itp-color-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function itpColorInfoDel(id, url){
