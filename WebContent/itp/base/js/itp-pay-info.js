...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/base/getPayInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpPayInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#itpPayInfoAdd").data('action-url');
		top.app.layer.editLayer('新增订单支付方式', ['710px', '220px'], '/itp/base/itp-pay-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function itpPayInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑订单支付方式', ['710px', '220px'], '/itp/base/itp-pay-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function itpPayInfoDel(id, url){
