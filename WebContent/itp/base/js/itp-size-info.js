...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/base/getSizeInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpSizeInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#itpSizeInfoAdd").data('action-url');
		top.app.layer.editLayer('新增商品尺码', ['710px', '220px'], '/itp/base/itp-size-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function itpSizeInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑商品尺码', ['710px', '220px'], '/itp/base/itp-size-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function itpSizeInfoDel(id, url){
