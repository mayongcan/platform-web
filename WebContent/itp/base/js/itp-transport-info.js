...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/base/getTransportInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpTransportInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#itpTransportInfoAdd").data('action-url');
		top.app.layer.editLayer('新增订单运输方式', ['710px', '220px'], '/itp/base/itp-transport-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function itpTransportInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑订单运输方式', ['710px', '220px'], '/itp/base/itp-transport-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function itpTransportInfoDel(id, url){
