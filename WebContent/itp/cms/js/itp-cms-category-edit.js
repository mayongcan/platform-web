...
function initTree(){
	...
	g_comboBoxTree.init($('#parentNode') , g_params.allTreeData, '535px');
}
...
function initView(){//判断是新增还是修改
	if(g_params.type == "edit"){
		...
		$('#name').val(g_params.node.data.name == undefined ? "" : g_params.node.data.name);
		$('#categoryDesc').val(g_params.node.data.categoryDesc == undefined ? "" : g_params.node.data.categoryDesc);
		$('#keywords').val(g_params.node.data.keywords == undefined ? "" : g_params.node.data.keywords);
		$('#dispOrder').val(g_params.node.data.dispOrder == undefined ? "" : g_params.node.data.dispOrder);

		$('#image').prettyFile({text:"请选择图片", placeholder:"若不需要修改，请留空"});
	}else{
...
function ajaxUploadImage(){
	...
	//上传图片到资源服务器
	top.app.uploadMultiImage($("#image")[0].files, function(data){
		g_imagePath = data;
		//提交数据
		submitAction();
	});
