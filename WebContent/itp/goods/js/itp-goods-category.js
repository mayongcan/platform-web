		...
		grid: {
			columns: [
				...
				{header: "类型", value: "type", title:"type", headerClass: 'jstree-grid-header-middle', width: 100, value: function(treeList){
					...
					else if(treeList.data.type == '2') return "商家自定义";
				}},
				{header: "所属商家", value: "merchantsName", title:"merchantsName", headerClass: 'jstree-grid-header-middle'},
...
function initFuncBtnEvent(){
	$("#itpGoodsCategoryAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		...
		params.operUrl = top.app.conf.url.apigateway + $("#itpGoodsCategoryAdd").data('action-url');
		top.app.layer.editLayer('新增商品分类', ['710px', '480px'], '/itp/goods/itp-goods-category-edit.html', params, function(){
   			//重新加载
			$treeView.jstree(true).refresh();
		});
    });
	$("#itpGoodsCategoryEdit").click(function () {
		if(g_selectNode == null ){
		...
		params.parentNode = getSelNodeParent();
		params.allTreeData = g_allTreeData;
		params.operUrl = top.app.conf.url.apigateway + $("#itpGoodsCategoryEdit").data('action-url');
		top.app.layer.editLayer('编辑商品分类', ['710px', '480px'], '/itp/goods/itp-goods-category-edit.html', params, function(){
   			//重新加载列表
			$treeView.jstree(true).refresh();
		});
    });
	$("#itpGoodsCategoryDel").click(function () {
		if(g_selectNode == null ){
		...
		var operUrl = top.app.conf.url.apigateway + $("#itpGoodsCategoryDel").data('action-url');
