	...
	$treeView.jstree({
		'core': {
			...
			'data': function (objNode, cb) {
				$.ajax({
				    url: top.app.conf.url.apigateway + "/api/itp/merchants/getMerchantsCategoryTreeList",
				    method: 'GET',
				    data: {
		...
		grid: {
			columns: [
				...
				{header: "类目编码", value: "categoryNum", title:"categoryNum", headerClass: 'jstree-grid-header-middle'},
				...
				{header: "创建人", value: "createByName", title:"createByName", headerClass: 'jstree-grid-header-middle'},
...
function initFuncBtnEvent(){
	$("#itpMerchantsCategoryAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		...
		params.operUrl = top.app.conf.url.apigateway + $("#itpMerchantsCategoryAdd").data('action-url');
		top.app.layer.editLayer('新增商品分类', ['710px', '550px'], '/itp/merchants/itp-merchants-category-edit.html', params, function(){
   			//重新加载
			$treeView.jstree(true).refresh();
		});
    });
	$("#itpMerchantsCategoryEdit").click(function () {
		if(g_selectNode == null ){
		...
		if(!canEdit){
			if(g_selectNode.data.type == "2" && g_selectNode.data.merchantsId == itp.getUserMerchantsId())
				canEdit = true;
		}
		//通过最总判断，是否具有权限
		if(!canEdit){
		...
		params.parentNode = getSelNodeParent();
		params.allTreeData = g_allTreeData;
		params.operUrl = top.app.conf.url.apigateway + $("#itpMerchantsCategoryEdit").data('action-url');
		top.app.layer.editLayer('编辑商品分类', ['710px', '550px'], '/itp/merchants/itp-merchants-category-edit.html', params, function(){
   			//重新加载列表
			$treeView.jstree(true).refresh();
		});
    });
	$("#itpMerchantsCategoryDel").click(function () {
		if(g_selectNode == null ){
		...
		var operUrl = top.app.conf.url.apigateway + $("#itpMerchantsCategoryDel").data('action-url');
