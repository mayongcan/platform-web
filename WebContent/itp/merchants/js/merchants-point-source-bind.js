...
function initFunc(){
	...
	for (var i = 0; i < length; i++) {
		//显示在列表上方的权限菜单
		if(g_operRights[i].dispPosition == '1' && g_operRights[i].funcFlag.indexOf("itpMerchantsPoint") != -1){
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpMerchantsPointBind").click(function () {
		//设置参数
		var params = {};
		params.merchantsId = g_params.row.id;
		params.operUrl = top.app.conf.url.apigateway + $("#itpMerchantsPointBind").data('action-url');
		top.app.layer.editLayer('选择积分源', ['900px', '550px'], '/itp/merchants/merchants-point-source-select.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function formatOperate(value, row, index){
	...
	var operateBtn = "";
	for (var i = 0; i < length; i++) {
		if(g_operRights[i].dispPosition == '2' && g_operRights[i].funcFlag.indexOf("itpMerchantsPoint") != -1){
...
function itpMerchantsPointUnbind(id, url){
	top.app.message.loading();
	//定义提交数据
	var submitData = {};
	submitData["idsList"] = id;
	...
	$.ajax({
		...
		success: function(data){
			...
			if(top.app.message.code.success == data.RetCode){
	   			top.app.message.notice("解除积分源绑定成功！");
				$table.bootstrapTable('refresh');
	   		}else{
