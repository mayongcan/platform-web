var $table = $('#tableList'), g_operRights = [], g_orderType = 'jhd', g_orderTypeName = '进货单';
...
function initFuncBtnEvent(){
	...
	$("#itpOrderJhdAdd").click(function () {
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderJhdAdd").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-jhd-edit.html?_pid=" + pid + "&backUrl=/itp/order/order-jhd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#itpOrderJhdEdit").click(function () {
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderJhdEdit").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-jhd-edit.html?_pid=" + pid + "&backUrl=/itp/order/order-jhd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#itpOrderJhdLock").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.postData($table, $("#itpOrderJhdLock").data('action-url'), JSON.stringify(obj), "确定要锁定当前选中的订单？", "订单锁定成功！");
    });
	$("#itpOrderJhdCancel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#itpOrderJhdCancel").data('action-url'), JSON.stringify(obj), "确定要取消当前选中的订单？", "订单取消成功！");
    });
	$("#itpOrderJhdFinish").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#itpOrderJhdFinish").data('action-url'), JSON.stringify(obj), "确定要完成当前选中的订单？", "订单操作成功！");
    });
	$("#itpOrderJhdDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 || rows.length > 1){
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderJhdDetail").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-jhd-detail.html?_pid=" + pid + "&backUrl=/itp/order/order-jhd-list.html";
		window.location.href = encodeURI(url);
    });
}
