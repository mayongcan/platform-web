...
function initView(){
	...
	$('#orderPayStatus').click(function () {
		...
		top.app.layer.editLayer('订单付款信息', ['900px', '550px'], '/itp/order/status-change-pay.html', params, function(retParams){
	...
	$('#orderSendStatus').click(function () {
		var params = {};
		params.orderInfo = g_params;
		top.app.layer.editLayer('订单发货信息', ['900px', '550px'], '/itp/order/status-change-send.html', params, function(retParams){
			if(retParams == null || retParams == undefined && retParams.length > 0) {
				top.app.message.alert("获取返回内容失败！");
				return;
			}
	...
	$('#orderReceiveStatus').click(function () {
		var params = {};
		params.orderInfo = g_params;
		top.app.layer.editLayer('订单发货信息', ['900px', '550px'], '/itp/order/status-change-receive.html', params, function(retParams){
			if(retParams == null || retParams == undefined && retParams.length > 0) {
				top.app.message.alert("获取返回内容失败！");
				return;
			}
	...
	}else if(g_params.orderTypeId == 'thd'){
...
function loadCustomer(){
	...
	if(g_params.row.customerId == '-1'){
		$('#customerInfo').text('未知客户');
	}else{
		if($.utils.isNull(g_customerInfo))
			g_customerInfo = itp.getCustomerById(g_params.row.customerId);
		$('#customerInfo').append('客户类型：<span style="margin-right:50px;">' + g_params.row.customerTypeName + '</span>');
		$('#customerInfo').append('客户等级：<span style="margin-right:50px;">' + g_params.row.customerLevelName + '</span>');
		$('#customerInfo').append('客户名称：<span style="margin-right:50px;">' + g_customerInfo.customerName + '</span>');
		$('#customerInfo').append('客户余额：<span style="margin-right:50px;color:#81c41f">' + accounting.formatMoney(g_customerInfo.customerBalance, "¥") + '</span>');
	}
}
