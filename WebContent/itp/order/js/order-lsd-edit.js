...
function initData(){
	if(g_params.type == 'edit'){
		...
		if(g_params.row.customerId == '-1'){
			...
			var customer = itp.getCustomerById(g_params.row.customerId);
...
function loadOtherInfo(){
	...
	$('#btnSelectSellerBy').click(function () {
		var params = {};
		params.shopId = getShopId();
		top.app.layer.editLayer('选择用户', ['900px', '550px'], '/itp/order/select-merchants-user.html', params, function(retParams){
			if(retParams == null || retParams == undefined && retParams.length > 0) {
				top.app.message.alert("获取返回内容失败！");
				return;
			}
