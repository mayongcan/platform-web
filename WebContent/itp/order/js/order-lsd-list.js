var $table = $('#tableList'), g_operRights = [], g_orderType = 'lsd', g_orderTypeName = '零售单';
...
function initFuncBtnEvent(){
	...
	$("#itpOrderLsdAdd").click(function () {
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderLsdAdd").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/itp/order/order-lsd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#itpOrderLsdEdit").click(function () {
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderLsdEdit").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/itp/order/order-lsd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#itpOrderLsdLock").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.postData($table, $("#itpOrderLsdLock").data('action-url'), JSON.stringify(obj), "确定要锁定当前选中的订单？", "订单锁定成功！");
    });
	$("#itpOrderLsdCancel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#itpOrderLsdCancel").data('action-url'), JSON.stringify(obj), "确定要取消当前选中的订单？", "订单取消成功！");
    });
	$("#itpOrderLsdFinish").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#itpOrderLsdFinish").data('action-url'), JSON.stringify(obj), "确定要完成当前选中的订单？", "订单操作成功！");
    });
	$("#itpOrderLsdDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 || rows.length > 1){
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderLsdDetail").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-lsd-detail.html?_pid=" + pid + "&backUrl=/itp/order/order-lsd-list.html";
		window.location.href = encodeURI(url);
    });
}
