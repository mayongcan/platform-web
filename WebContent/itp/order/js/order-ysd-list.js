var $table = $('#tableList'), g_operRights = [], g_orderType = 'ysd', g_orderTypeName = '预售单';
...
function initView(){
	...
	g_orderPayStatusDict = top.app.getDictDataByDictTypeValue('ITP_ORDER_PAY_STATUS');
	g_orderSendStatusDict = top.app.getDictDataByDictTypeValue('ITP_ORDER_SEND_STATUS');
	g_orderReceiveStatusDict = top.app.getDictDataByDictTypeValue('ITP_ORDER_RECEIVE_STATUS');
	...
	itp.getCustomerType($("#searchCustomerType"), itp.getUserMerchantsId(), true);
	itp.getCustomerLevel($("#searchCustomerLevel"), itp.getUserMerchantsId(), true);
}
...
function initTable(){
	...
	var searchParams = function (params) {
        var param = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			orderSendStatus: $("#searchOrderSendStatus").val(),
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/order/getOrderInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	...
	$("#itpOrderYsdAdd").click(function () {
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderYsdAdd").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/itp/order/order-ysd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#itpOrderYsdEdit").click(function () {
		...
		if(rows[0].orderStatus == '2'){
			top.app.message.alert("当前订单已锁定，不能进行编辑！");
			return;
		}
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderYsdEdit").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/itp/order/order-ysd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#itpOrderYsdLock").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.postData($table, $("#itpOrderYsdLock").data('action-url'), JSON.stringify(obj), "确定要锁定当前选中的订单？", "订单锁定成功！");
    });
	$("#itpOrderYsdCancel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#itpOrderYsdCancel").data('action-url'), JSON.stringify(obj), "确定要取消当前选中的订单？", "订单取消成功！");
    });
	$("#itpOrderYsdFinish").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#itpOrderYsdFinish").data('action-url'), JSON.stringify(obj), "确定要完成当前选中的订单？", "订单操作成功！");
    });
	$("#itpOrderYsdDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 || rows.length > 1){
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpOrderYsdDetail").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/itp/order/order-lsd-detail.html?_pid=" + pid + "&backUrl=/itp/order/order-ysd-list.html";
		window.location.href = encodeURI(url);
    });
}
