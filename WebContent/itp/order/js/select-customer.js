...
function initView(){
	...
	var searchParams = function (p) {
        var retParam = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			customerAddr: $("#searchCustomerAddr").val(),
        };
        return retParam;
    };
    //初始化列表
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/customer/getCustomerInfoList",   		//请求后台的URL（*）
...
function submitAction(){
	...
	var rowObj = [];
	rowObj.customerId = g_selectRowsId.id;
	rowObj.customerName = g_selectRowsId.customerName;
	rowObj.customerTypeId = g_selectRowsId.typeId;
	rowObj.customerTypeName = g_selectRowsId.typeName;
	rowObj.customerLevelId = g_selectRowsId.levelId;
	rowObj.customerLevelName = g_selectRowsId.levelName;
	rowObj.customerBalance = g_selectRowsId.customerBalance;
