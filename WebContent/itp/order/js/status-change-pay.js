...
function initView(){
	...
	//加载已付款列表
	var orderPayList = itp.getOrderPayList(g_params.orderInfo.row.id);
...
function submitData(){
	...
	//异步处理
	$.ajax({
		url: top.app.conf.url.apigateway + "/api/itp/order/editOrderPay?access_token=" + top.app.cookies.getCookiesToken(),
	    method: 'POST',
