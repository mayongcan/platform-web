...
function initView(){
	if(g_params.orderInfo.row.orderSendStatus == '3'){
		...
		for(var i = 0; i < goodsList.length; i++){
			...
			var goodsDetailList = itp.getOrderGoodsDetailByDetailId(goodsList[i].id, "0", "");	//获取未发货的
			var goodsDataList = [];
	...
	//加载发货记录列表
	var sendList = itp.getOrderSendLogList(g_params.orderInfo.row.id);
...
function submitAction(){
	...
	//异步处理
	$.ajax({
		url: top.app.conf.url.apigateway + "/api/itp/order/addOrderSendLog?access_token=" + top.app.cookies.getCookiesToken(),
	    method: 'POST',
