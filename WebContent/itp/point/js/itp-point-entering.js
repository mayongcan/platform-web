...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/point/getPointEnteringList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpPointEnteringAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#itpPointEnteringAdd").data('action-url');
		top.app.layer.editLayer('积分源手动录入结款', ['710px', '340px'], '/itp/point/itp-point-entering-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function itpPointEnteringEdit(id, url){
	...
	top.app.layer.editLayer('编辑内容', ['710px', '340px'], '/itp/point/itp-point-entering-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function itpPointEnteringDel(id, url){
