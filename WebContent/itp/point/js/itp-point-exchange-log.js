...
function initTable(){
	...
	var searchParams = function (params) {
        var param = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			pointSourceName: $("#searchPointSourceName").val(),
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/point/getPointExchangeLogList",   		//请求后台的URL（*）
...
function itpPointExchangeLogDetail(id, url){
	var row = $table.bootstrapTable("getRowByUniqueId", id);
}

function formatStatus(value, row, index){
	if(value == '1') return '<font color="green">成功</font>';
	else return '<font color="red">失败</font>';
}
