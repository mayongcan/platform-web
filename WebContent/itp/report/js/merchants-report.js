		...
		success: function(data) {
			if(top.app.message.code.success == data.RetCode) {
				if(data.RetData != null && data.RetData != undefined && data.RetData.length > 0) {
					top.app.addComboBoxOption($("#searchMerchantsId"), data.RetData, true);
...
function initTable(){
	...
	var searchParams = function (params) {
        var param = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            merchantsId: $("#searchMerchantsId").val(),
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/report/getReportMerchantsCharge",   		//请求后台的URL（*）
