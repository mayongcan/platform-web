...
function initSearchPanel(){
	...
	//获取用户列表
	$.ajax({
		url: top.app.conf.url.apigateway + "/api/itp/user/getUserKeyVal",
		method: 'GET',
		data: {
		...
		success: function(data) {
			if(top.app.message.code.success == data.RetCode) {
				if(data.RetData != null && data.RetData != undefined && data.RetData.length > 0) {
					top.app.addComboBoxOption($("#searchUserId"), data.RetData, true);
...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/itp/report/getReportUserExpense",   		//请求后台的URL（*）
...
function tableFormatPayResult(value, row){
	var payTotal = row.payTotal;
	var deductTotal = row.deductTotal;
	if($.utils.isEmpty(payTotal)) payTotal = 0;
	if($.utils.isEmpty(deductTotal)) deductTotal = 0;
	return accounting.formatMoney(payTotal - deductTotal, "¥");
}
