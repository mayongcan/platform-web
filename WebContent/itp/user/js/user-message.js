...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#itpUserMessageAdd").click(function () {
		//设置传送对象
		top.app.info.iframe.params = {};
		top.app.info.iframe.params.type = 'add';
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpUserMessageAdd").data('action-url');
		
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "user-message-edit.html?_pid=" + pid;
		window.location.href = encodeURI(url);
    });
	$("#itpUserMessageEdit").click(function () {
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpUserMessageEdit").data('action-url');
		
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "user-message-edit.html?_pid=" + pid;
		window.location.href = encodeURI(url);
    });
	$("#itpUserMessageDel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.delData($table, $("#itpUserMessageDel").data('action-url'), idsList);
    });
	$("#itpUserMessageSend").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.postData($table, $("#itpUserMessageSend").data('action-url'), idsList,
				"确定发送当前选中的消息？", "消息发送成功！");
    });
	$("#itpUserMessageRevoke").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
			top.app.message.alert("请选择要撤回的消息！");
			return;
		}
		var idsList = "";
		var hasSend = false;
		...
		if(!hasSend){
			top.app.message.alert("未发送的消息不能撤回！");
			return;
		}
		appTable.postData($table, $("#itpUserMessageRevoke").data('action-url'), idsList,
				"确定撤回当前选中的消息？", "消息撤回成功！");
    });
	$("#itpUserMessageDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 || rows.length > 1){
		...
		if(rows[0].sendDate != null && rows[0].sendDate != undefined && rows[0].sendDate != '' ){
			...
			top.app.info.iframe.params = {};
			top.app.info.iframe.params.rows = rows[0];
			top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#itpUserMessageDetail").data('action-url');
			
			var pid = $.utils.getUrlParam(window.location.search,"_pid");
			var url = "user-message-detail.html?_pid=" + pid;
			window.location.href = encodeURI(url);
		}else{
			top.app.message.alert("当前选中的消息未发送！");
			return;
		}
    });
}
