...
//	parent.app.layer.retParams.push(rowObj);
	...
	if(g_selectRowsId == null || g_selectRowsId == undefined || g_selectRowsId.length == 0){
		top.app.message.notice("请选择需要关联的文书！");
		return;
	}
	//获取当前选中日常巡查下所关联的所有文书
	var rowObj = [];
	//rowObj.relevanceIdList = g_selectRowsId.id + "";
	rowObj.relevanceIdList = g_selectRowsId.relevanceIdList + "";
	rowObj.relevanceCodeList = g_selectRowsId.rapCode;
	parent.app.layer.retParams = [];
	parent.app.layer.retParams.push(rowObj);
