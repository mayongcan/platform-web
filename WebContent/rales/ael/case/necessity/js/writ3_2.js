...
var g_codeType = rales.writNecessity3_2, g_codeCurNum = "";
...
function initView(){
	$.date.initSearchDate('divInquiryBeginDate', 'divInquiryEndDate', "YYYY-MM-DD");
	//1新增 2编辑 3查看
	if(g_params.type == 1){
		//增加表单验证
		formValidate();
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#inquiryEndDate').val(g_params.subRow.content.inquiryEndDate);
			$('#lawUser1').val(g_params.subRow.content.lawUser1);
			$('#lawUser2').val(g_params.subRow.content.lawUser2);
			$('#content1').val(g_params.subRow.content.content1);
			$('#content2').val(g_params.subRow.content.content2);
			$('#content3').val(g_params.subRow.content.content3);
		}
		//显示文书列表
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdInquiryEndDate').text($.utils.getNotNullVal(g_params.subRow.content.inquiryEndDate));
			...
			$('#tdContent1').text($.utils.getNotNullVal(g_params.subRow.content.content1));
			$('#tdContent2').text($.utils.getNotNullVal(g_params.subRow.content.content2));
			$('#tdContent3').text($.utils.getNotNullVal(g_params.subRow.content.content3));
		}
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/necessity/writ-pr3_2.html', params, function(){});
    });
	//提交
	...
	$("#btnPreview").click(function () {
		...
		var params = {};
		params.data = getTableParams();
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/necessity/writ-pre3_2.html', params, function(){});
    });
	//返回
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){	
		...
		data.content2 = $('#content2').val();
		data.content3 = $('#content3').val();
	}else{
