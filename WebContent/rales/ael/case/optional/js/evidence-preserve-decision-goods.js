...
function initTable(){
	...
	var searchParams = function (params) {
        var param = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            preservationId: g_params.preservationId
        };
        return param;
    };
    //初始化列表
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/rales/ael/case/getEvidenceGoodsList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	$("#newEvent").click(function () {
		...
		params.preservationId = g_params.preservationId;
		params.operUrl = top.app.conf.url.apigateway + "/api/rales/ael/case/addEvidenceGoods";
		top.app.layer.editLayer('新增物品', ['710px', '400px'], '/rales/ael/case/optional/evidence-preserve-decision-goods-edit.html', params, function(){
   			//重新加载列表
	...
	$("#editEvent").click(function () {
		...
		params.preservationId = g_params.preservationId;
		params.rows = rows[0];
		params.operUrl = top.app.conf.url.apigateway + "/api/rales/ael/case/editEvidenceGoods";
		top.app.layer.editLayer('编辑物品', ['710px', '400px'], '/rales/ael/case/optional/evidence-preserve-decision-goods-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
	$("#delEvent").click(function () {
		...
		appTable.delData($table, "/api/rales/ael/case/delEvidenceGoods", idsList);
    });
}
