...
var g_codeType = "22", g_codeCurNum = "";
...
function initView(){
	...
	$('#tdRecorder').text(hearingNoticeInfo.courtClerkName);
	$('#tdDate').text($.date.dateFormat(hearingNoticeInfo.hearingDay, "yyyy-MM-dd"));
	...
	if(g_params.type == 1){
		...
		$('#hearingDetail').val(g_params.subRow.baseInfo);
		$('#hearingConclusion').val(g_params.subRow.conclusion);
		$('#suggest').val(g_params.subRow.approvalOpinion);
		...
		$('#tdHearingDetail').text($.utils.getNotNullVal(g_params.subRow.baseInfo));
		$('#tdHearingConclusion').text($.utils.getNotNullVal(g_params.subRow.conclusion));
		$('#tdSuggest').text($.utils.getNotNullVal(g_params.subRow.approvalOpinion));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/hearing-report-pre.html', params, function(){});
    });
	//提交
...
function getTableParams(){
	...
	data.tdHearingCompere = $('#tdHearingCompere').text();
	data.tdRecorder = $('#tdRecorder').text();
	data.tdDate = $('#tdDate').text();
	data.tdAddress = $('#tdAddress').text();
	if(g_params.type == 1 || g_params.type == 2){
		data.hearingDetail = $('#hearingDetail').val();
		data.hearingConclusion = $('#hearingConclusion').val();
		data.suggest = $('#suggest').val();
		data.memo = $('#memo').val();
	}else{
		data.hearingDetail = $('#tdHearingDetail').text();
		data.hearingConclusion = $('#tdHearingConclusion').text();
...
function formValidate(){
	$("#divEditForm").validate({
        rules: {
        	...
        	hearingConclusion: {required: true},
...
function submitAction(){
	...
	var url = "";
	if(g_params.type == 2){
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/editHearingReport?access_token=" + top.app.cookies.getCookiesToken(),
		submitData["id"] = g_params.id;
	} else{
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/addHearingReport?access_token=" + top.app.cookies.getCookiesToken(),
	...
	submitData["recorder"] = $("#tdRecorder").text();
	submitData["recordDate"] = $("#tdDate").text();

	submitData["baseInfo"] = $("#hearingDetail").val();
	submitData["conclusion"] = $("#hearingConclusion").val();
	submitData["approvalOpinion"] = $("#suggest").val();
