...
var g_codeType = "21", g_codeCurNum = "";
$(function () {
...
function initView(){
	...
	if(g_params.type == 1){
		...
		$('#reason').val(g_params.subRow.reason);
		...
		$('#tdReason').text($.utils.getNotNullVal(g_params.subRow.reason));
	...
	$("#btnPrint").click(function () {
		top.app.message.chooseEvent("打印选择", "请选择打印项", "打印第一联", "打印第二联",function(){
			...
			top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/no-hearing-notice-pre.html', params, function(){});
		}, function(){
			var params = {};
			params.isPrint = true;
			params.printType = 2;
...
function submitAction(){
	...
	var url = "";
	if(g_params.type == 2){
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/editNoHearing?access_token=" + top.app.cookies.getCookiesToken(),
		submitData["id"] = g_params.id;
	} else{
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/addNoHearing?access_token=" + top.app.cookies.getCookiesToken(),
