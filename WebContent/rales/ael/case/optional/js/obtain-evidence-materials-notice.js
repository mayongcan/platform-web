...
function initView(){
	...
	if(g_params.type == 1){
		...
		$('#rules').val(g_params.subRow.lawGist);
		$('#obtainEvidence').val(g_params.subRow.materialContent);
		$('#deadline').val($.date.dateFormat(g_params.subRow.endDate, "yyyy-MM-dd"));
		$('#postalUnit').val(g_params.subRow.mailCompany);
		$('#address').val(g_params.subRow.address);
		$('#zip').val(g_params.subRow.zip);
		...
		$('#tdRules').text($.utils.getNotNullVal(g_params.subRow.lawGist));
		$('#tdObtainEvidence').text($.utils.getNotNullVal(g_params.subRow.materialContent));
		$("#tdDeadline").text($.date.dateFormat(g_params.subRow.endDate, "yyyy-MM-dd"));
		$('#tdPostalUnit').text($.utils.getNotNullVal(g_params.subRow.mailCompany));
		$('#tdAddress').text($.utils.getNotNullVal(g_params.subRow.address));
		$('#tdZip').text($.utils.getNotNullVal(g_params.subRow.zip));
	...
	$("#btnPrint").click(function () {
		top.app.message.chooseEvent("打印选择", "请选择打印项", "打印第一联", "打印第二联",function(){
			...
			top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/obtain-evidence-materials-notice-pre.html', params, function(){});
		}, function(){
			var params = {};
			params.isPrint = true;
			params.printType = 2;
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		...
		data.obtainEvidence = $('#obtainEvidence').val();
		data.deadline = $('#deadline').val();
		data.postalUnit = $('#postalUnit').val();
		...
		data.obtainEvidence = $('#tdObtainEvidence').text();
		data.deadline = $('#tdDeadline').text();
		data.postalUnit = $('#tdPostalUnit').text();
...
function formValidate(){
	$("#divEditForm").validate({
        rules: {
    		...
    		rules: {required: true},
    		obtainEvidence: {required: true},
    		postalUnit: {required: true},
...
function submitAction(){
	...
	var url = "";
	if(g_params.type == 2){
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/editForensicMaterials?access_token=" + top.app.cookies.getCookiesToken(),
		submitData["id"] = g_params.id;
	} else{
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/addForensicMaterials?access_token=" + top.app.cookies.getCookiesToken(),
	...
	submitData["lawGist"] = $("#rules").val();
	submitData["materialContent"] = $("#obtainEvidence").val();
	submitData["endDate"] = $("#deadline").val();
	submitData["mailCompany"] = $("#postalUnit").val();
