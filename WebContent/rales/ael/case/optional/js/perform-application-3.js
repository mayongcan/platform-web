...
function initView(){
	...
	if(g_params.type == 1){
		...
		$("#ruleRows").val(g_params.subRow.strip);
		$("#ruleWord").val(g_params.subRow.item);
		$("#ruleMark").val(g_params.subRow.mark);
		...
		$('#tdRuleRows').text($.utils.getNotNullVal(g_params.subRow.strip));
		$('#tdRuleWord').text($.utils.getNotNullVal(g_params.subRow.item));
		$('#tdRuleMark').text($.utils.getNotNullVal(g_params.subRow.mark));
	...
	$("#btnPrint").click(function () {
		top.app.message.chooseEvent("打印选择", "请选择打印项", "打印第一联", "打印第二联",function(){
			...
			top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/perform-application-pre-3.html', params, function(){});
		}, function(){
			var params = {};
			params.isPrint = true;
			params.printType = 2;
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		...
		data.ruleRows = $('#ruleRows').val();
		data.ruleWord = $('#ruleWord').val();
		data.ruleMark = $('#ruleMark').val();
		...
		data.ruleRows = $('#tdRuleRows').text();
		data.ruleWord = $('#tdRuleWord').text();
		data.ruleMark = $('#tdRuleMark').text();
...
function formValidate(){
	$("#divEditForm").validate({
        rules: {
    		...
    		ruleRows: {required: true},
    		ruleWord: {required: true},
    		ruleMark: {required: true},
...
function submitAction(){
	...
	submitData["strip"] = $("#ruleRows").val();
	submitData["item"] = $("#ruleWord").val();
	submitData["mark"] = $("#ruleMark").val();
