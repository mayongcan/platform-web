...
var g_codeType = "14", g_codeCurNum = "";
...
function initView(){
	$('#divRemoveBeginDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true});
	$('#divStorageDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true});
	...
	if(g_params.type == 1){
		...
		$('#removeBeginDate').val($.date.dateFormat(g_params.subRow.beginDate, "yyyy-MM-dd"));
		$('#storageDate').val($.date.dateFormat(g_params.subRow.relieveDate, "yyyy-MM-dd"));

		g_relevanceIdList = $.utils.getNotNullVal(g_params.subRow.preservationId);
		g_relevanceCodeList = $.utils.getNotNullVal(g_params.subRow.preservationCode);
		...
		$('#tdRemoveBeginDate').text($.date.dateFormat(g_params.subRow.beginDate, "yyyy-MM-dd"));
		$("#tdStorageDate").text($.date.dateFormat(g_params.subRow.relieveDate, "yyyy-MM-dd"));
	...
	$("#btnPrint").click(function () {
		top.app.message.chooseEvent("打印选择", "请选择打印项", "打印第一联", "打印第二联",function(){
			...
			top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/remove-evidence-preserve-decision-pre.html', params, function(){});
		}, function(){
			var params = {};
			params.isPrint = true;
			params.printType = 2;
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		data.removeBeginDate = $('#removeBeginDate').val();
		data.storageDate = $('#storageDate').val();
		data.code = $('#preservationId').val();
	}else{
		data.removeBeginDate = $('#tdRemoveBeginDate').text();
		data.storageDate = $('#tdStorageDate').text();
		data.code = $('#tdPreservationId').text();
	}
	return data;
}
...
function formValidate(){
	$("#divEditForm").validate({
        rules: {	
        	removeBeginDate: {required: true, dateISO:true},
        	storageDate: {required: true, dateISO:true}
        },
        messages: {
        },
        //重写showErrors
...
function submitAction(){
	...
	var url = "";
	if(g_params.type == 2){
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/editRemoveEvidencePreserveDecision?access_token=" + top.app.cookies.getCookiesToken(),
		submitData["id"] = g_params.id;
	} else{
		url = top.app.conf.url.apigateway + "/api/rales/ael/case/addRemoveEvidencePreserveDecision?access_token=" + top.app.cookies.getCookiesToken(),
	...
	submitData["beginDate"] = $('#removeBeginDate').val();
	submitData["relieveDate"] = $('#storageDate').val();
