...
var g_codeType = rales.writOptional10_4, g_codeCurNum = "";
...
function initView(){
	...
	$('#divCarrayOutDate').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true, defaultDate: new Date()});
	//1新增 2编辑 3查看
	if(g_params.type == 1){
		//增加表单验证
		formValidate();
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#pushWrit').val(g_params.subRow.content.pushWrit);
			$('#carrayOutDate').val(g_params.subRow.content.carrayOutDate);
			$('#protocolContent').val(g_params.subRow.content.protocolContent);
			...
			$('#applyInfo').val(g_params.subRow.content.applyInfo);
			$('#handleContent').val(g_params.subRow.content.handleContent);
			$('#fileContent').val(g_params.subRow.content.fileContent);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdPushWrit').text($.utils.getNotNullVal(g_params.subRow.content.pushWrit));
			$('#tdCarrayOutDate').text($.utils.getNotNullVal(g_params.subRow.content.carrayOutDate));
			$('#tdProtocolContent').text($.utils.getNotNullVal(g_params.subRow.content.protocolContent));
			...
			$('#tdApplyInfo').text($.utils.getNotNullVal(g_params.subRow.content.applyInfo));
			$('#tdHandleContent').text($.utils.getNotNullVal(g_params.subRow.content.handleContent));
			$('#tdFileContent').html($.utils.getNotNullVal(g_params.subRow.content.fileContent).replace(/\n/g,'<br/>'));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pr10_4.html', params, function(){});
    });
	//提交
	...
	$("#btnPreview").click(function () {
		...
		var params = {};
		params.data = getTableParams();
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre10_4.html', params, function(){});
    });
	//返回
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){	
		...
		data.pushWrit = $('#pushWrit').val();
		data.carrayOutDate = $('#carrayOutDate').val();
		data.protocolContent = $('#protocolContent').val();
		...
		data.applyInfo = $('#applyInfo').val();
		data.handleContent = $('#handleContent').val();
		data.fileContent = $('#fileContent').val();
