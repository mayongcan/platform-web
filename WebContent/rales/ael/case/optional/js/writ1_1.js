...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			top.app.addCheckBoxButton($("#divCheckUserAdvice"), g_checkRegisterAdviceDict, 'checkUserAdvice', g_params.subRow.content.checkUserAdvice, true);
			...
			$('#checkDate').val(g_params.subRow.content.checkDate);
			$('#mainCheckDetail').val(g_params.subRow.content.mainCheckDetail);
		}
		//显示文书列表
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdCheckDate').text($.utils.getNotNullVal(g_params.subRow.content.checkDate));
			$('#tdMainCheckDetail').text($.utils.getNotNullVal(g_params.subRow.content.mainCheckDetail));
		}
		...
		if(g_params.caseIsNormalCase == '1')
			getHistoryAuditList(g_params.row.id, "11");
		else
			getHistoryAuditList(g_params.row.id, "10");
	}
	...
	$("#btnPrint").click(function () {
		...
		params.taskSourceDict = g_taskSourceDict;
		params.checkRegisterAdviceDict = g_checkRegisterAdviceDict;
		params.data = getTableParams();
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre1_1.html', params, function(){});
    });
	//预览
