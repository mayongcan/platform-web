 ...
 */
function initView(){
	$('#divDealline').datetimepicker({locale: 'zh-CN', format: 'YYYY-MM-DD', allowInputToggle: true, defaultDate: new Date()});
	//判断是新增还是修改
	if(g_params.type == "edit"){
		$('#name').val(g_params.rows.name);
		$('#totalCnt').val(g_params.rows.totalCnt);
		$('#type').val(g_params.rows.type);
		$('#addr').val(g_params.rows.addr);
		$('#deadline').val(g_params.rows.deadline);
	}
}
...
function submitAction(){
	...
	rowObj.type = $("#type").val();
	rowObj.addr = $("#addr").val();
	rowObj.deadline = $("#deadline").val();
