...
var g_codeType = rales.writOptional2_2, g_codeCurNum = "";
...
function initView(){
	...
	$.date.initSearchDate('divSaveDateBegin', 'divSaveDateEnd', "YYYY-MM-DD");
	//1新增 2编辑 3查看
	if(g_params.type == 1){
		//增加表单验证
		formValidate();
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#saveDateBegin').val(g_params.subRow.content.saveDateBegin);
			$('#saveDateEnd').val(g_params.subRow.content.saveDateEnd);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdSaveDateBegin').text($.utils.getNotNullVal(g_params.subRow.content.saveDateBegin));
			$('#tdSaveDateEnd').text($.utils.getNotNullVal(g_params.subRow.content.saveDateEnd));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pr2_2.html', params, function(){});
    });
	//提交
	...
	$("#btnPreview").click(function () {
		...
		var params = {};
		params.data = getTableParams();
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre2_2.html', params, function(){});
    });
	//返回
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){	
		...
		data.saveDateBegin = $('#saveDateBegin').val();
		data.saveDateEnd = $('#saveDateEnd').val();
