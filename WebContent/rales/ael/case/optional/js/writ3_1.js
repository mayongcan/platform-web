...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$("input[type='radio'][name=auditType][value=" + g_params.subRow.content.auditType + "]").attr("checked",true);
		...
		if(g_params.caseIsNormalCase == '1')
			getHistoryAuditList(g_params.row.id, "15");
		else
			getHistoryAuditList(g_params.row.id, "14");
	}
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre3_1.html', params, function(){});
    });
	//预览
