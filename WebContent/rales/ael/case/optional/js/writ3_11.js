...
var g_codeType = rales.writOptional3_11, g_codeCurNum = "";
...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#rule').val(g_params.subRow.content.rule);
			...
			$("input[type='radio'][name=relieveType][value=" + g_params.subRow.content.relieveType + "]").attr("checked",true);
			$('#relieveName').val(g_params.subRow.content.relieveName);
			$('#returnContent1').val(g_params.subRow.content.returnContent1);
			$('#returnContent2').val(g_params.subRow.content.returnContent2);
			$('#returnContent3').val(g_params.subRow.content.returnContent3);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdRule').text($.utils.getNotNullVal(g_params.subRow.content.rule));
			...
			$("input[type='radio'][name=relieveType][value=" + g_params.subRow.content.relieveType + "]").attr("checked",true);
			$('#tdRelieveName').text($.utils.getNotNullVal(g_params.subRow.content.relieveName));
			$('#tdReturnContent1').text($.utils.getNotNullVal(g_params.subRow.content.returnContent1));
			$('#tdReturnContent2').text($.utils.getNotNullVal(g_params.subRow.content.returnContent2));
			$('#tdReturnContent3').text($.utils.getNotNullVal(g_params.subRow.content.returnContent3));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre3_11.html', params, function(){});
    });
	//提交
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){	
		...
		data.relieveType = $('#tdRelieveType input:radio:checked').val();
		data.relieveName = $('#relieveName').val();
		data.returnContent1 = $('#returnContent1').val();
		data.returnContent2 = $('#returnContent2').val();
		data.returnContent3 = $('#returnContent3').val();
