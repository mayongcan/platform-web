...
var g_codeType = rales.writOptional9_2, g_codeCurNum = "";
...
function initView(){
	...
	if(g_params.type == 1){
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('punishWrit').val(g_params.subRow.content.punishWrit);
		...
		if(!$.utils.isNull(g_params.subRow.content)){
			...
			$('#tdPunishWrit').text($.utils.getNotNullVal(g_params.subRow.content.punishWrit));
	...
	$("#btnPrint").click(function () {
		...
		top.app.layer.editLayer('预览', ['725px', '600px'], '/rales/ael/case/optional/writ-pre9_2.html', params, function(){});
    });
	//预览
...
function getTableParams(){
	...
	if(g_params.type == 1 || g_params.type == 2){
		...
		data.punishWrit = $('#punishWrit').val();
