...
function initView(){
	...
	if(g_params.row.activityName == '行政强制措施草稿' || g_params.row.activityName == '重新编辑' || g_params.row.activityName == '重新编辑1'){
		$('#btnAudit').text("提交");
...
function initNavButton(){
	...
	$('#btnAudit').click(function () {
		...
		$.ajax({
			url: top.app.conf.url.apigateway + "/api/rales/ael/force/getTodoList?access_token=" + top.app.cookies.getCookiesToken(),
		    method: 'GET',
			data:JSON.stringify(),
		...
		success: function(data){
			if(top.app.message.code.success == data.RetCode){
				...
				for(var i = 0; i < length; i++){
					var editButton = "";
					if(g_params.isEdit){
						if(g_params.removeAuditEdit && index == 3 && subIndex == 1){
							//移除审批表的编辑
						}else{
