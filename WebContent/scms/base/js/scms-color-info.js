...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/base/getColorInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsColorInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsColorInfoAdd").data('action-url');
		top.app.layer.editLayer('新增商品颜色', ['710px', '220px'], '/scms/base/scms-color-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsColorInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑商品颜色', ['710px', '220px'], '/scms/base/scms-color-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsColorInfoDel(id, url){
