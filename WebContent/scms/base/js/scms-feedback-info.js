...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/base/getFeedbackInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsFeedbackInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsFeedbackInfoAdd").data('action-url');
		top.app.layer.editLayer('新增反馈信息', ['710px', '320px'], '/scms/base/scms-feedback-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsFeedbackInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑反馈信息', ['710px', '320px'], '/scms/base/scms-feedback-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsFeedbackInfoDel(id, url){
