...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/base/getSizeGroupList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsSizeGroupAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsSizeGroupAdd").data('action-url');
		top.app.layer.editLayer('新增尺码分组', ['900px', '500px'], '/scms/base/scms-size-group-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsSizeGroupEdit(id, url){
	...
	top.app.layer.editLayer('编辑尺码分组', ['900px', '500px'], '/scms/base/scms-size-group-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsSizeGroupDel(id, url){
