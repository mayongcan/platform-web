...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsSizeInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsSizeInfoAdd").data('action-url');
		top.app.layer.editLayer('新增商品尺码', ['710px', '220px'], '/scms/base/scms-size-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsSizeInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑商品尺码', ['710px', '220px'], '/scms/base/scms-size-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsSizeInfoDel(id, url){
