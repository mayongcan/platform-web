...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/base/getTextureInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsTextureInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsTextureInfoAdd").data('action-url');
		top.app.layer.editLayer('新增商品材质', ['710px', '220px'], '/scms/base/scms-texture-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsTextureInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑商品材质', ['710px', '220px'], '/scms/base/scms-texture-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsTextureInfoDel(id, url){
