...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/base/getTransportInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsTransportInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsTransportInfoAdd").data('action-url');
		top.app.layer.editLayer('新增订单运输方式', ['710px', '220px'], '/scms/base/scms-transport-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsTransportInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑订单运输方式', ['710px', '220px'], '/scms/base/scms-transport-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsTransportInfoDel(id, url){
