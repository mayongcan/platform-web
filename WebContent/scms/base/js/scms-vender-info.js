...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/base/getVenderInfoList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsVenderInfoAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsVenderInfoAdd").data('action-url');
		top.app.layer.editLayer('新增厂家', ['710px', '480px'], '/scms/base/scms-vender-info-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsVenderInfoEdit(id, url){
	...
	top.app.layer.editLayer('编辑厂家', ['710px', '480px'], '/scms/base/scms-vender-info-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsVenderInfoDel(id, url){
