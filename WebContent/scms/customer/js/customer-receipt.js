...
function initView(){
	...
	$('#customerName').text(g_params.rows.customerName);
	$('#customerBalance').text(accounting.formatMoney(g_params.rows.customerBalance, "¥"));
...
function submitAction(){
	...
	submitData["orderCustomerType"] = '1';
	submitData["customerId"] = g_params.rows.id;
	submitData["customerName"] = g_params.rows.customerName;
	//如果不是合并支付，则客户余额不变，订单按照实际支付情况显示结果
	if($('#checkIsCombinePay').prop('checked')) {
		//账户余额=客户余额 + (已使用账号支付的金额 - 当前账号需要支付的金额)
		submitData["customerBalance"] = g_params.rows.customerBalance  - g_orderTotalPay;
