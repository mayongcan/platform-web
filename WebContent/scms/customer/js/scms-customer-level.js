...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/customer/getCustomerLevelList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsCustomerLevelAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsCustomerLevelAdd").data('action-url');
		top.app.layer.editLayer('新增客户等级', ['710px', '320px'], '/scms/customer/scms-customer-level-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsCustomerLevelEdit(id, url){
	...
	top.app.layer.editLayer('编辑客户等级', ['710px', '320px'], '/scms/customer/scms-customer-level-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsCustomerLevelDel(id, url){
