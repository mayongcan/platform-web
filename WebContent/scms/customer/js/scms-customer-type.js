...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/customer/getCustomerTypeList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsCustomerTypeAdd").click(function () {
		//设置参数
		var params = {};
		params.type = 'add';
		params.operUrl = top.app.conf.url.apigateway + $("#scmsCustomerTypeAdd").data('action-url');
		top.app.layer.editLayer('新增客户类型', ['710px', '220px'], '/scms/customer/scms-customer-type-edit.html', params, function(){
   			//重新加载列表
			$table.bootstrapTable('refresh');
		});
    });
...
function scmsCustomerTypeEdit(id, url){
	...
	top.app.layer.editLayer('编辑客户类型', ['710px', '220px'], '/scms/customer/scms-customer-type-edit.html', params, function(){
			//重新加载列表
		$table.bootstrapTable('refresh');
	});
}

function scmsCustomerTypeDel(id, url){
