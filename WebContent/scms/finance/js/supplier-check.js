...
function initView(){
	...
	$.typeahead({
	    ...
	    template: function (query, item) {
	    	...
	    	if(!$.utils.isEmpty(item.supplierPhoto)){
	    		var imageList = item.supplierPhoto.split(',');
	    	...
	    	return '<div style="margin-right:15px;float:left">' +
		            ...
		            '<div style="float:left">' +
		            	'<div style="margin-right:15px;font-size:14px;font-weight: bold;">供货商名称：{{supplierName}}</div>' + 
		            	'<div class="searchResult">手机号码：{{supplierPhone}}</div>' + 
		            '</div>' +
	    ...
	    source: {
	    	showGoodsList: {
	    		display: ["supplierName", "supplierPhone"],
	            ajax: function (query) {
	                return {
	                    type: "GET",
	    ...
	    callback: {
	        ...
	        onClickAfter: function (node, a, item, event) {
	        	$('#searchNameAndPhone').val(item.supplierName);
	        },
	...
	$("#btnSearch").click(function () {
		if(g_selectData == null){
			top.app.message.notice("请选择需要对账的供货商！");
			return;
		}
		top.app.message.loading();
		loadAllInfo();
...
function loadBaseInfo(){
	$('#formTitle').text("供货商对账单-" + ($('#searchCheckType').val() == '1' ? "订单概况" : '订单明细'));
	$('#supplierName').text("供货商名称：" + g_selectData.supplierName);
	$('#supplierBalance').text("供货商余额：" + accounting.formatMoney(g_selectData.supplierBalance, "¥"));
	$('#supplierPhone').text("手机号码：" + g_selectData.supplierPhone);
