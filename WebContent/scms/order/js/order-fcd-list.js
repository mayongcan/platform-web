...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/order/getOrderJhdList",   		//请求后台的URL（*）
...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsOrderFcdAdd").click(function () {
		//进入创建订单页面
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderFcdAdd").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-jhd-edit.html?_pid=" + pid + "&backUrl=/scms/order/order-fcd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#scmsOrderFcdEdit").click(function () {
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderFcdEdit").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-jhd-edit.html?_pid=" + pid + "&backUrl=/scms/order/order-fcd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#scmsOrderFcdLock").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.postData($table, $("#scmsOrderFcdLock").data('action-url'), JSON.stringify(obj), "确定要锁定当前选中的订单？", "订单锁定成功！");
    });
	$("#scmsOrderFcdCancel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#scmsOrderFcdCancel").data('action-url'), JSON.stringify(obj), "确定要取消当前选中的订单？", "订单取消成功！");
    });
	$("#scmsOrderFcdFinish").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#scmsOrderFcdFinish").data('action-url'), JSON.stringify(obj), "确定要完成当前选中的订单？", "订单操作成功！");
    });
	$("#scmsOrderFcdDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 || rows.length > 1){
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderFcdDetail").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-jhd-detail.html?_pid=" + pid + "&backUrl=/scms/order/order-fcd-list.html";
		window.location.href = encodeURI(url);
    });
}
