...
function initData(){
	if(g_params.type == 'edit'){
		//加载供货商
		var supplier = scms.getSupplierById(g_params.row.customerId);
...
function loadCustomer(){
	$('#btnSelectOrderCustomerId').click(function () {//设置参数
		var params = {};
		top.app.layer.editLayer('选择供货商', ['900px', '550px'], '/scms/order/select-supplier.html', params, function(retParams){
			if(retParams == null || retParams == undefined && retParams.length > 0) {
				top.app.message.alert("获取返回内容失败！");
				return;
			}
