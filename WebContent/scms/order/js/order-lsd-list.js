...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsOrderLsdAdd").click(function () {
		//进入创建订单页面
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderLsdAdd").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/scms/order/order-lsd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#scmsOrderLsdEdit").click(function () {
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderLsdEdit").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/scms/order/order-lsd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#scmsOrderLsdLock").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.postData($table, $("#scmsOrderLsdLock").data('action-url'), JSON.stringify(obj), "确定要锁定当前选中的订单？", "订单锁定成功！");
    });
	$("#scmsOrderLsdCancel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#scmsOrderLsdCancel").data('action-url'), JSON.stringify(obj), "确定要取消当前选中的订单？", "订单取消成功！");
    });
	$("#scmsOrderLsdFinish").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#scmsOrderLsdFinish").data('action-url'), JSON.stringify(obj), "确定要完成当前选中的订单？", "订单操作成功！");
    });
	$("#scmsOrderLsdDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 || rows.length > 1){
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderLsdDetail").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-lsd-detail.html?_pid=" + pid + "&backUrl=/scms/order/order-lsd-list.html";
		window.location.href = encodeURI(url);
    });
}
