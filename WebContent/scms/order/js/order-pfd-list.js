...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsOrderPfdAdd").click(function () {
		//进入创建订单页面
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderPfdAdd").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/scms/order/order-pfd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#scmsOrderPfdEdit").click(function () {
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderPfdEdit").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-lsd-edit.html?_pid=" + pid + "&backUrl=/scms/order/order-pfd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#scmsOrderPfdLock").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		appTable.postData($table, $("#scmsOrderPfdLock").data('action-url'), JSON.stringify(obj), "确定要锁定当前选中的订单？", "订单锁定成功！");
    });
	$("#scmsOrderPfdCancel").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#scmsOrderPfdCancel").data('action-url'), JSON.stringify(obj), "确定要取消当前选中的订单？", "订单取消成功！");
    });
	$("#scmsOrderPfdFinish").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 ){
		...
		obj.idsList = idsList;
		obj.orderType = g_orderType;
		appTable.postData($table, $("#scmsOrderPfdFinish").data('action-url'), JSON.stringify(obj), "确定要完成当前选中的订单？", "订单操作成功！");
    });
	$("#scmsOrderPfdDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		if(rows.length == 0 || rows.length > 1){
		...
		top.app.info.iframe.params.orderSendStatusDict = g_orderSendStatusDict;
		top.app.info.iframe.params.orderReceiveStatusDict = g_orderReceiveStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderPfdDetail").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-lsd-detail.html?_pid=" + pid + "&backUrl=/scms/order/order-pfd-list.html";
		window.location.href = encodeURI(url);
    });
}
