...
function initFuncBtnEvent(){
	//绑定工具条事件
	$("#scmsOrderSydAdd").click(function () {
		//进入创建订单页面
		...
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderSydAdd").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
		var url = "/scms/order/order-syd-new.html?_pid=" + pid + "&backUrl=/scms/order/order-syd-list.html";
		window.location.href = encodeURI(url);
    });
	$("#scmsOrderSydCancel").click(function () {
		...
		appTable.postData($table, $("#scmsOrderSydCancel").data('action-url'), JSON.stringify(obj), "确定要取消当前选中的订单？", "订单取消成功！");
    });
	$("#scmsOrderSydDetail").click(function () {
		var rows = appTable.getSelectionRows($table);
		...
		top.app.info.iframe.params.orderTypeDict = g_orderTypeDict;
		top.app.info.iframe.params.orderStatusDict = g_orderStatusDict;
		top.app.info.iframe.params.operUrl = top.app.conf.url.apigateway + $("#scmsOrderSydDetail").data('action-url');
		var pid = $.utils.getUrlParam(window.location.search,"_pid");
