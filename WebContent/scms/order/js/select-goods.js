...
function loadGenInventoryInfo(){
	...
	var tableContent = "", colorRowSpan = g_selectSize.length;
	...
	for(var colorIndex = 0; colorIndex < colorLength; colorIndex++){
		...
		for(var sizeIndex = 0; sizeIndex < sizeLength; sizeIndex++){
			...
			var tmpId = "_" + selColorId + "_" + selSizeId;
			//显示库存
			...
			if(selColorId != '' && selSizeId != ''){
				for(var shopIndex = 0; shopIndex < g_params.shopList.length; shopIndex++){
					...
					for(var inventoryIndex = 0;inventoryIndex < g_goodsInventoryDataList.length; inventoryIndex++){
						if(g_params.shopList[shopIndex].ID == g_goodsInventoryDataList[inventoryIndex].shopId && 
								g_goodsInventoryDataList[inventoryIndex].colorId == selColorId && 
								g_goodsInventoryDataList[inventoryIndex].inventorySizeId == selSizeId) {
			...
			tableContent += '<td class="reference-td" style="line-height:20px;">' + 
							...
							'</td>' + 
							'<td class="reference-td" style="line-height:20px;" id="inventoryNum' + tmpId + '" >' + 
								inventoryRet + 
							'</td>' + 
	...
	setTimeout(function () {
		...
		for(var colorIndex = 0; colorIndex < g_selectColor.length; colorIndex++){
			for(var sizeIndex = 0; sizeIndex< g_selectSize.length; sizeIndex++){
				var tmpId = "_" + g_selectColor[colorIndex].id + "_" + g_selectSize[sizeIndex].id;
		...
		//计算总金额：单价 * 折扣 * 数量
		var discount = scms.getDicount($('#defDiscount').val());
		//判断计算总金额类型
		if(g_params.orderTypeId == 'lsd' || g_params.orderTypeId == 'pfd' || g_params.orderTypeId == 'ysd' 
			|| g_params.orderTypeId == 'thd')
		...
		for(var sizeIndex = 0; sizeIndex< g_selectSize.length; sizeIndex++){
			...
			if(document.getElementById("orderNum" + tmpId)){
				...
				if(trimRet != "" && !isNaN(trimRet) && parseInt(trimRet) != 0) {
					...
					//利润=订单单价*折扣 - 成本
					var discount = scms.getDicount(obj.goodsDiscount);
					...
					obj.inventoryNum = $("#inventoryNum" + tmpId).text();
					dataList.push(obj);
				}
			}
		}
