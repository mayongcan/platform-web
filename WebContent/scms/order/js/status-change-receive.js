...
function initView(){
	if(g_params.orderInfo.row.orderReceiveStatus == '3'){
		...
		for(var i = 0; i < goodsList.length; i++){
			...
			var goodsDetailList = scms.getOrderGoodsDetailByDetailId(goodsList[i].id, "", "0");	//获取未收货的
			var goodsDataList = [];
	...
	//加载发货记录列表
	var receiveList = scms.getOrderReceiveLogList(g_params.orderInfo.row.id);
...
//	submitData["orderReceiveStatus"] = $('#divOrderReceiveStatus input:radio:checked').val();
	...
	//异步处理
	$.ajax({
		url: top.app.conf.url.apigateway + "/api/scms/order/addOrderReceiveLog?access_token=" + top.app.cookies.getCookiesToken(),
	    method: 'POST',
