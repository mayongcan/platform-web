 ...
 */
function initView(){
	if(g_params.orderInfo.row.orderSendStatus == '3'){
		$('#divGoodsList').text('当前订单的商品已全部发货！');
		$('#divSubmitButton').remove();
		$('#tdSendMemo').remove();
	}else{
		//加载商品列表
		var goodsList = scms.getOrderGoodsByOrderId(g_params.orderInfo.row.id);
		for(var i = 0; i < goodsList.length; i++){
			...
			var goodsDetailList = scms.getOrderGoodsDetailByDetailId(goodsList[i].id, "0", "");	//获取未发货的
			var goodsDataList = [];
	...
	//加载发货记录列表
	var sendList = scms.getOrderSendLogList(g_params.orderInfo.row.id);
	$('#tbodyOrderSendRecord').empty();
	for(var i = 0; i < sendList.length; i++){
		$('#tbodyOrderSendRecord').append('<tr>' + 
												'<td class="reference-td">' + sendList[i].sendByName + '</td>' + 
												'<td class="reference-td">' + sendList[i].sendDate + '</td>' + 
												'<td class="reference-td">' + sendList[i].sendMemo + '</td>' + 
											'</tr>');
	}
...
function submitAction(){
	...
	if(idList == ''){
		top.app.message.notice("请选择需要发货的商品！");
		return;
	}
	if($('#sendMemo').val() == ''){
...
//	submitData["orderSendStatus"] = $('#divOrderSendStatus input:radio:checked').val();
	submitData["sendMemo"] = $('#sendMemo').val();
	//获取发货列表
	submitData["idList"] = idList;
	top.app.message.loading();
	//异步处理
	$.ajax({
		url: top.app.conf.url.apigateway + "/api/scms/order/addOrderSendLog?access_token=" + top.app.cookies.getCookiesToken(),
	    method: 'POST',
		...
		success: function(data){
			...
			if(top.app.message.code.success == data.RetCode){
				//数据提交成功后，返回信息
				var rowObj = [];
				g_params.orderInfo.row.orderSendStatus = data.RetData;
