...
function initSearchPanel(){
	//搜索面板高度
	appTable.searchPannelHeight = $('#searchDetailTitle').outerHeight(true) + $('#searchPannel').outerHeight(true) + 70;	
	//bootstrap 折叠事件，替换图标
    $('.collapse-content').on('show.bs.collapse', function (e) {
    	appTable.searchPannelHeight += 95;
...
function initTable(){
	...
	var searchParams = function (params) {
        var param = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
            ...
            merchantsId: scms.getUserMerchantsId(),
            orderTypeList: 'lsd,pfd,ysd,thd,jhd,fcd',
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/report/getSalePurchaseCompareList",   		//请求后台的URL（*）
...
function tableFormatIsEmpty(value,row,index){
...
function getStatistics(){
	$.ajax({
		url: top.app.conf.url.apigateway + "/api/scms/report/getSalePurchaseCompareStatistics",
	    method: 'GET',
	   	data:{
	    	access_token: top.app.cookies.getCookiesToken(),
            merchantsId: scms.getUserMerchantsId(),
	   	...
	   	success: function(data){
	   		if(top.app.message.code.success == data.RetCode){
	   			if(data.rows != null && data.rows != undefined && data.rows.length > 0){
	   				$('#totalPurchase').text($.utils.isEmpty(data.rows[0].totalPurchase) ? 0 : data.rows[0].totalPurchase);
	   				$('#totalReturn').text($.utils.isEmpty(data.rows[0].totalReturn) ? 0 : data.rows[0].totalReturn);
	   				$('#totalSale').text($.utils.isEmpty(data.rows[0].totalSale) ? 0 : data.rows[0].totalSale);
	   				$('#totalSaleReturn').text($.utils.isEmpty(data.rows[0].totalSaleReturn) ? 0 : data.rows[0].totalSaleReturn);
	   			}else{
	   				$('#totalPurchase').text(0);
	   				$('#totalReturn').text(0);
	   				$('#totalSale').text(0);
	   				$('#totalSaleReturn').text(0);
	   			}
	   		}
	   	}
	});
