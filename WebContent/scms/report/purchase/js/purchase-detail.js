...
function initTable(){
	...
	$table.bootstrapTable({
        url: top.app.conf.url.apigateway + "/api/scms/report/getPurchaseReportDetailList",   		//请求后台的URL（*）
...
function tableFormatTotalPrice(value,row,index){
	var retVal = row.goodsOrderPrice * row.goodsOrderNum;
	if(row.orderType == 'fcd')
		return '<font color="red">' + accounting.formatMoney(-retVal, "¥") + '</font>';
	else return accounting.formatMoney(retVal, "¥");
}

function tableFormatDiscountTotalPrice(value,row,index){
	var retVal = row.goodsOrderPrice * scms.getDicount(row.goodsDiscount) * row.goodsOrderNum;
	if(row.orderType == 'fcd')
		return '<font color="red">' + accounting.formatMoney(-retVal, "¥") + '</font>';
	else return accounting.formatMoney(retVal, "¥");
}
